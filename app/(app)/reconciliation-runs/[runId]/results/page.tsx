...
export default function ResultsPage() {
  ...
  async function exportWorkbook() { setError(""); setExporting(true); try { const blob = await api.exportReconciliationWorkbook(params.runId); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `Novoriq_Reconciliation_Report_${params.runId}.xlsx`; anchor.click(); URL.revokeObjectURL(url); } catch { setError("Novoriq could not export this report. Please try again."); } finally { setExporting(false); } }
